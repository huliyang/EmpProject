...
/**
 *  定义一个公共的DAO操作接口标准，基本的功能包括增加、修改全部、删除数据、根据编号查询、查询全部、分页显示、数据统计
 * @author 胡立阳
 *
...
public interface IDAO<K,V> {
	 ...
	 * 根据编号id查询指定的信息
	 * @param id 要查询的编号
	 * @return 如果信息存在，则将数据以VO类对象的形式返回，如果数据不存在，则返回null
	 * @throws Exception SQL执行异常
	 */
	public V findById(K id) throws Exception;
	/**
	 * 查询指定数据表的全部数据，并且以集合的形式返回
	 ...
	 * 如果表中有数据，则所有的数据会封装为VO对象而后利用List集合返回，<br>
	 * 如果没有数据，那么集合的长度为0（size()==0，不是null）
